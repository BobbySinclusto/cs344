...
// This function will modify the value of pt and key!
void encrypt(char *pt, char *key) {
   // Loop through each character
   for (int i = 0; i < strlen(pt); ++i) {
      // Replace space characters with values that make the encryption easier
      if (pt[i] == ' ') pt[i] = 'A' + 26;
      if (key[i] == ' ') key[i] = 'A' + 26;
      pt[i] = (pt[i] + key[i] - 2 * 'A') % 27 + 'A';
...
bool handshake(int connectionSocket) {
   // Send "enc" to start the handshake
...
int main(int argc, char *argv[]){
   ...
   while(1){
      ...
      if (child_pid == 0) {
         ...
         int ptlen = recv_int(connectionSocket);
         // Receive the plaintext
         char *pt = malloc(sizeof(char) * (ptlen + 1));
         recv_string(connectionSocket, pt, ptlen);
         
         // Encrypt the message
         encrypt(pt, key);

         // Send the encrypted text back to the client
         send_string(connectionSocket, pt, ptlen);
