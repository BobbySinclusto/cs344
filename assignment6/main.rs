...
fn main() {
    ...
    let mut thread_list: Vec<std::thread::JoinHandle<usize>> = Vec::with_capacity(2);
    for part in xs {
        thread_list.push(thread::spawn(move || map_data(&part)));
    }
    for t in thread_list {
        intermediate_sums.push(t.join().expect("Thread failed to join."));    
    }
    ...
    let mut thread_list: Vec<std::thread::JoinHandle<usize>> = Vec::with_capacity(num_partitions);
    for part in partitions {
        thread_list.push(thread::spawn(move || map_data(&part)));
    }
    // 4. Collects the intermediate sums from all the threads
    let mut sums = Vec::with_capacity(num_partitions);
    for t in thread_list {
        sums.push(t.join().expect("Thread failed to join."));    
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    let mut partitions: Vec<Vec<usize>> = Vec::with_capacity(num_partitions);

    // Figure out how many elements to put in normally sized partitions
    let element_count: usize = v.len() / num_partitions;

    // Figure out how many normally sized partitions there should be
    let num_normal: usize = num_partitions - (v.len() % num_partitions);

    // Add partitions with correct number of elements
    for i in 0..num_normal {
        let mut part: Vec<usize> = Vec::with_capacity(element_count);
        for j in i * element_count..(i + 1) * element_count {
            part.push(v[j]);
        }
        partitions.push(part);
    }
    // Add partitions with extra elements
    for i in 0..num_partitions - num_normal {
        let mut part: Vec<usize> = Vec::with_capacity(element_count);
        for j in num_normal * element_count + i * (element_count + 1)..num_normal * element_count + (i + 1) * (element_count + 1) {
            part.push(v[j]);
        }
        partitions.push(part);
    }
